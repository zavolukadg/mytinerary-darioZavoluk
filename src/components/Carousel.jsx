...
export default function Carousel() {
    ...
    function next(){
        if(indiceFinal >= carousel.length){
            setIndiceInicial(0);
            setIndiceFinal(4)
        }
        else{
    ...
    function prev(){
        if(indiceInicial == 0){
            setIndiceInicial(carousel.length-4);
            setIndiceFinal(carousel.length)
        }
        else{
            setIndiceInicial(indiceInicial-4);
            setIndiceFinal(indiceFinal-4) 
        ...
        <div className="flex flex-col w-[400px] justify-center">
            ...
            <div className="flex">
                ...
                <div className="flex justify-center flex-col sm:flex-row flex-wrap">
                    {carousel.slice(indiceInicial, indiceFinal).map((each, index) => <CarouselCard key={index} src={each.photo} alt={each.id} text={each.city} />)}
                </div>
